...
from roop.utilities import clean_dir, detect_fps, is_video
#from roop.main import CFG

def main():
    parser = argparse.ArgumentParser(description="Face Swap Script")
    parser.add_argument("--input", required=False, help="Path to the input image or video")
    parser.add_argument("--target", required=False, help="Path to the target image or video")
    ...
    process_entry = ProcessEntry(args.target, 0, get_video_frame_total(args.target) if is_video(args.target) else 1, detect_fps(args.target) if is_video(args.target) else 0)
